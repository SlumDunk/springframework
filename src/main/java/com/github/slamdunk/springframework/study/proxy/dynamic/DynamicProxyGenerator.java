package com.github.slamdunk.springframework.study.proxy.dynamic;
...
/**
 * a generator for creating dynamic class
 * 
 * @author liuzhongda
 *
 */
public class DynamicProxyGenerator {
	...
	private static void createProxy() throws Exception {
		...
		CtClass cc = pool.makeClass("com.github.slamdunk.springframework.study.proxy.dynamic.StationProxy");

		// 设置接口
		CtClass interface1 = pool.get("com.github.slamdunk.springframework.study.proxy.facade.TicketService");
		cc.setInterfaces(new CtClass[] { interface1 });

		// 设置Field
		CtField field = CtField.make("private com.github.slamdunk.springframework.study.proxy.vo.Station station;",
				cc);

		cc.addField(field);

		CtClass stationClass = pool.get("com.github.slamdunk.springframework.study.proxy.vo.Station");
